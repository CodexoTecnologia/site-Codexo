...
export default function Culture() {
  ...
  return (
    <section id="manifesto" className="relative z-10 py-32 bg-codexo-dark overflow-hidden border-t border-white/5">
      <div className="container mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
        ...
        <div className="relative flex justify-center items-center h-[600px]">
          ...
          <div className="absolute w-[450px] h-[450px] bg-codexo-primary/20 blur-[120px] rounded-full z-0 animate-pulse" />

          {/* NÚCLEO CENTRAL - BORDA MAIS GROSSA E BRILHO */}
          <div className="relative w-80 h-80 bg-codexo-dark-light clip-pentagon border-2 border-white/20 backdrop-blur-3xl flex items-center justify-center z-20 shadow-[0_0_50px_rgba(102,126,234,0.25)]">
            <motion.div 
              animate={{ rotate: 360 }}
              transition={{ duration: 25, repeat: Infinity, ease: "linear" }}
              className="absolute inset-4 border-2 border-codexo-primary/40 clip-pentagon"
            />
            ...
            <AnimatePresence mode="wait">
              <motion.div
                ...
                exit={{ opacity: 0, y: -15 }}
                className="text-center px-10 z-30"
              >
                <span className="text-codexo-primary font-black text-[11px] tracking-[0.6em] uppercase block mb-4">
                  VALOR 0{activePilar + 1}
                </span>
                <h4 className="text-white font-black text-4xl uppercase italic tracking-tighter leading-none drop-shadow-2xl">
          ...
          {[0, 72, 144, 216, 288].map((angle, i) => (
            <div key={i} className="absolute inset-0 flex items-center justify-center">
              ...
              <motion.div 
                ...
                className="absolute h-[2px] origin-left left-1/2 w-[205px]"
                style={{ transform: `rotate(${angle - 90}deg)` }}
              />

              <motion.div
                className="absolute cursor-pointer z-40"
                style={{ transform: `rotate(${angle}deg) translateY(-235px) rotate(-${angle}deg)` }}
              ...
              >
                <motion.div 
                  ...
                  className="w-16 h-16 clip-pentagon flex items-center justify-center border-2 transition-all duration-300 shadow-2xl"
                >
                  <span className={`font-black text-lg ${activePilar === i ? "text-white" : "text-codexo-primary"}`}>
                    0{i + 1}
                  </span>
                </motion.div>
              </motion.div>
        ...
        <div className="lg:pl-12 space-y-10">
          <div className="space-y-4">
            <h3 className="text-5xl md:text-7xl font-black text-white leading-[0.8] uppercase tracking-tighter">
              NOSSA <br />
          ...
          >
            <p className="text-slate-300 text-lg md:text-2xl leading-relaxed font-medium border-l-4 border-codexo-primary pl-8 italic">
          ...
          <div className="flex gap-4 pt-4 border-t border-white/5">
             <div className="flex flex-col">
                <span className="text-white font-black text-[10px] uppercase">Missão</span>
                <span className="text-slate-500 text-[9px] uppercase tracking-wider">Escalando o sucesso através da performance</span>
             </div>
          </div>
        </div>
      </div>
